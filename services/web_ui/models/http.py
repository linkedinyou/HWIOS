...
from twisted.web.http import _IdentityTransferDecoder,HTTPChannel, HTTPFactory
...
class HWMFactory(Site):
    ...
    def log(self, request):
        if hasattr(self, "logFile"):
            line = '%s %s/%s\n' % (self._escape(request.uri), self._escape(request.clientproto),self._escape(request.method))
...
class WebSocketSite(HWMFactory):
    """
