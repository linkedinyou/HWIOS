# -*- coding: utf-8 -*-
"""
    services.web_ui.models.infinote
    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

    A python port of the jinfinote library

    JInfinote
    
    :copyright: 2009 Simon Veith <simon@jinfinote.com>
    :license: MIT, see https://github.com/sveith/jinfinote/blob/master/LICENSE
...
class InfinotePool(object):
    """
    The infinote pool is used to track document's and their changes by infinote text-editing clients
    """
    def __init__(self, ws_handler):
        self.ws_handler = ws_handler
    
    
    def subscribe(self, client, item_id, initial_text, app_pool, remote_callback, client_role = 'edit'):
        """General pool subscriber for infinote clients

        :param Client client: The requesting client
        :param str item_id: The id of the item specifies an editor environment for that item in an app_pool
        :param str app_pool: General pool describer, for instance 'wiki'
        :param def remote_callback: A function that's called for callback afterwards
        """        
        online = []
        if  not hasattr(client,'color'):
            client.color = "#%06x" % random.randint(0,16777215)
        ...
        if item_id not in HWIOS.ws_realm.pool.subscription[app_pool]:
            ...
            HWIOS.ws_realm.pool.subscription[app_pool][item_id]['editor'].try_insert([1, '', 0, str(initial_text)])
        ...
        #Add this client to the subscription list after some cleaning up
        ...
        return {'state':state,'log':log_history,'online':online,'pool_id':item_id}
    ...
    def request_insert(self, client, app_pool, item_id, params, remote_callback):
        """Infinote client wants to perform a insert operation on the subscription log

        :param Client client: The requesting client
        :param str app_pool: General pool describer, for instance 'wiki'
        :param str item_id: The id of the item specifies an editor environment for that item in an app_pool
        :param dict params: The infinote parameters to use with this operation
        :param def remote_callback: A function that's called for callback afterwards
        """
    ...
    def request_delete(self, client, app_pool, item_id, params, remote_callback):
        ...
        for target_client in HWIOS.ws_realm.pool.subscription[app_pool][item_id]['clients']: 
            if client != target_client:
                remote_callback(target_client, app_pool, item_id, 'remove', params)
    ...
    def update_caret(self, client, app_pool, item_id, params, remote_callback):
        """Editing client wants to notify others of moving a cursor

        :param Client client: The requesting client
        :param str app_pool: General pool describer, for instance 'wiki'
        :param str item_id: The id of the item specifies an editor environment for that item in an app_pool
        :param dict params: The cursor parameters to use
        :param def remote_callback: A function that's called for callback afterwards
        """
        for target_client in HWIOS.ws_realm.pool.subscription[app_pool][item_id]['clients']: 
            if client != target_client:
                if target_client.role == 'edit':
                    remote_callback(target_client, app_pool, item_id, params)
                
                
    def get_other_clients(self, client, app_pool, item_id):
        """Get a list of all clients connected to a certain editor environment
        
        :param Client client: The requesting client
        :param str app_pool: General pool describer, for instance 'wiki'
        :param str item_id: The id of the item specifies an editor environment for that item in an app_pool
        :return: list - A list of clients connected to that editor environment
        """
        _clientlist = []
        for _client in HWIOS.ws_realm.pool.subscription[app_pool][item_id]['clients']: 
            if client != _client:
    ...
    def get_state(self, app_pool, item_id):
        """Get the document's state of a certain editor environment

        :param str app_pool: General pool describer, for instance 'wiki'
        :param str item_id: The id of the item specifies an editor environment for that item in an app_pool
        :return: str - The document's current state
        """
        return HWIOS.ws_realm.pool.subscription[app_pool][item_id]['editor'].get_state()
        

class InfinoteEditor(object):
    """The actual editor with functions to operate on the text, using infinote OT"""
    ...
    def try_insert(self, params):
        ...
        :param dict params: The parameters used for the insert operation [user, vector, position, text]
        """
        #user, text
        params[1] = params[1].encode("utf-8")
        params[3] = params[3].encode("utf-8")
    ...
    def try_delete(self, params):
        ...
        :param dict params: The parameters used for the delete operation [user, vector, position, text_length]
        """ 
        params[1] = params[1].encode("utf-8")
    ...
    def try_undo(self, params):
        ...
        :param dict params: The parameters used for the undo operation [user]
        """ 
    ...
    def sync(self):
        """Synchronizes a document based on it's log file"""
        for log in self.log:
            if log[0] == 'i': 
    ...
    def get_state(self):
        ...
        :return: str - The document's current text-state
        """
        return (self._state.vector.toString(), self._state.buffer.toString()) 
        
    
    def get_log(self, limit = None):
        """Get the document's log

        :return: list - The document's log of operations
        """
        if limit != None:
...
#TODO: Won't be documented for now beneath this line. Too lowlevel and not enough time
