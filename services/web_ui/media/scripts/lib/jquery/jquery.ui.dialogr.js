 ...
 * Developed by Carlos Carvalhar (http://carvalhar.com) with the help of Ciro Anuncia��o (http://voxelmidia.com.br) when working at PixFly (http://pixfly.com.br)
 *
...
$.widget("ui.dialogr", {
	...
	_create: function() {
		...
		var self = this,
			...
			titleId = $.ui.dialogr.getTitleId(this.element),
	...
	open: function() {
		...
		if (is_chrome) {
			var obj = this;
			setTimeout(function(){obj.restore();}, 100);
		}
	...
	minimize: function() {		
		...
		this.size();
		this.uiDialog.css({position:"absolute", left:"10px", width: 250, height: 100, bottom:"-60px"});
		this.uiDialog.css('position','fixed'); /* sticky the dialog at the page to avoid scrolling */
		$('#dialog-restore').css('display', 'block');
	},
	...
	maximize: function() {		
		...
		marginWDialog = $(window.body).width()-marginWDialog;
	...
	adjustScrollContent: function () {
		...
		$('.ui-dialog-content').css({width: widthDialog, height: heightDialog});
		$('#dialog').css({width: widthDialog, height: heightDialog});
	},
